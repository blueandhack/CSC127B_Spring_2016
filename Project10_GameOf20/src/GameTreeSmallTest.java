...
public class GameTreeSmallTest {
	...
	public void testGameWithSevenNodes() {
		...
		assertEquals("--rattlesnake\n-Is it a mammal?\n--tiger\nHas feathers?\n--owl\n-Barnyard?\n--chicken",
				aGame.toString());
	}

	// Test for filename is not exist
	@Test
	public void testEmptyFile() {
		GameTree aGame = new GameTree("xxx.txt");
		assertEquals("", aGame.getCurrent());
		aGame.playerSelected(Choice.No);
		aGame.saveGame();

		GameTree aGameTwo = new GameTree(null);
		assertEquals("", aGameTwo.getCurrent());
		aGame.saveGame();
	}
